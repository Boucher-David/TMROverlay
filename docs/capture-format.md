...
- `capture-synthesis.json` (currently synthesis version 2) summarizes the raw capture schema and sampled frame values for investigation without reopening the full binary payload. It is bounded by `TelemetryCapture:MaxSynthesisMilliseconds`; its normalized session context includes `DriverInfo.DriverCarIdx` so a diagnostic can distinguish session-declared local identity from transient player/focus telemetry.
