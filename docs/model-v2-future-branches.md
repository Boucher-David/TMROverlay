...
- 2026-07-14: Fuel production-cutover audit: Design V2 styling is already the default native shell, but Fuel V1 calculation still owns live history selection and advice. The unconditional static Fuel V2 laps-workbench interception is now disabled in product paths; a default-off `FuelV2Overlay:Enabled` developer gate instead presents only fresh local factual capacity, accepted burn-window, and range output through localhost/OBS and the sectioned Windows-native renderer. It exposes no learned-history selection, target, fuel-add, plan, or stint schedule and does not replace V1 while false. Do not cut over V1 strategy until V2 owns evidence selection/hysteresis, box-to-service-to-exit route facts, lifecycle/interruption policy, a real stint scheduler, geometry/settings/evidence parity, deterministic browser-review presenter replay, and Windows validation.
