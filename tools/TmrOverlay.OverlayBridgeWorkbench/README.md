...
```text
synthetic fixture → virtual circuit → ephemeral mutual TLS → Core frame/CBOR decode
                  → receiver admission → atomic composition → source-neutral Fuel input
```
...
dependency, persistence, credential/key store, or secret input. The short-lived certificates exist
in memory only. It is not a `LocalhostOverlays` or OBS route, and it is not evidence for a
...
It has no authenticated room binding, user pairing/approval, Oracle relay, persisted identity,
monotonic-clock source, real publisher telemetry adapter, multi-viewer fan-out, diagnostics, or
app-overlay consumption. The product Settings tab remains inert and truthful while this workbench
exists.
