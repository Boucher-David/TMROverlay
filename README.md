...
Raw telemetry capture is normally opt-in. This `v1.3` development branch deliberately sets `TelemetryCapture:RawCaptureEnabled=true` and `FuelV2Overlay:Enabled=true` so Windows testing automatically preserves enhanced-capture evidence and opens the factual V2 Fuel overlay. **Before merging any PR from this branch, restore both settings to `false`**; releases must continue to require the Support checkbox or an explicit configuration override, and retain V1 Fuel as the default. The enabled V2 overlay remains factual-only: it has no lower-half strategy, pit request, or fuel-add advice. In Test and Practice it can show a temporary Model Readiness matrix for exact car/layout evidence and hides that matrix once the factual collection goals are complete; it is not strategy permission. A narrowly verified grid/pit fallback may show only current/max Fuel State when the session-declared non-spectator driver exactly matches the raw camera but progress focus is absent. Normal diagnostics are compact summaries and logs; diagnostics bundles intentionally exclude raw `telemetry.bin` and source `.ibt` files. Fuel V2 calibration capture writes compact, source-qualified session sidecars under `fuel-v2-capture/`; format-7 retains stationary-service observations plus two-sample-confirmed local pit-entry/stall/exit route checkpoints, session-declared pit-location/pit-speed provenance, and requested tire shape with actual total/side/axle/corner counters when exposed. It records a shared cumulative/low-water refill signal and distinguishes a genuine request mutation from iRacing's normal post-service request clear. It never uses team-wide pit state or invents a pit-stall ordinal. Exact tire history can say an outcome was observed; service timing remains unadvised until a later qualified learner has a separately verified execution-rule source and comparable component-shape evidence. Selected derived facts are promoted separately into `history/user/fuel-v2/`. Fuel V2 history is grouped by exact car + exact track layout and explicit session family; clean Offline Testing is a separate quality-gated `test` family that is practice-like but never relabeled as race. Race length and fuel-cap/BoP facts stay contextual for the future live selector. Version-1 mixed connection records are retained as diagnostic-only legacy evidence. `FuelV2History:UseForStrategy` remains inactive in this branch, so V1 fuel strategy does not consume any Fuel V2 history.

The Test/Practice matrix is collection guidance, not strategy permission: it checks a stopped pit route, small/large measured fills, fuel-only and fuel-with-tires samples, plus exact `1 tire`/front/rear/left/right/four-tire service outcomes. A continuous no-stall pit-lane pass is retained as optional travel calibration and never keeps the matrix visible. Raw requests and `DCRuleSet` stay stored provenance rather than user-facing readiness cells.
...
- Browser-review screenshots are produced with `npm run screenshots:browser-review` and uploaded by CI under `browser-review-screenshots`.
- Localhost route screenshots, including supported OBS route aliases, are produced separately with `npm run screenshots:localhost` and uploaded by CI under `localhost-screenshots`.
- Legacy tracked review screenshots still live under `mocks/`, but they are not a release parity gate. Use `--profile legacy-mock-slices` or `--profile legacy-contact-sheets` only when deliberately auditing those retired mock assets.
- Windows-rendered screenshots are generated by CI from real WinForms forms and uploaded under `windows-overlay-screenshots`; installer menu screenshots are uploaded under `windows-installer-screenshots`. CI also publishes a `windows-screenshots` bundle with both sets for side-by-side collection. For local Windows collection, pass an MSI path as the second argument to `tools/TmrOverlay.WindowsScreenshots` to write installer screenshots under the same output folder's `installer/` subfolder.
- Pull-request runs upload a short-lived `tmroverlay-pr-<number>-installer` artifact containing the MSI created by the Velopack dry run, alongside the portable test-build artifact. It is for manual tester installation only; it is not a GitHub Release asset or an in-app updater channel.
...
npm run screenshots:localhost
python3 tools/validate_overlay_screenshots.py --profile localhost-ci --root artifacts/localhost-screenshots
git diff --check
...
For a combined local browser-review plus localhost capture, use `npm run screenshots:all` and validate it with `--profile browser-localhost-ci`. For legacy mock cleanup, run `python3 tools/validate_overlay_screenshots.py --profile legacy-mock-slices` or `--profile legacy-contact-sheets`; failures there should be handled as retirement/audit work, not browser/localhost/Windows parity failures.
