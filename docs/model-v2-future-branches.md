...
These gates, policy/lease binding tests, rate-limited/bounded claim replay handling, and real Windows transport validation are required before Oracle provisioning is considered an implementation task. A 2026-07-19 adversarial review reaffirmed this boundary: a green PR build is the next checkpoint, not permission to connect a throwaway browser or LAN transport to Oracle. First complete one production-shaped, deterministic vertical slice (platform-kept identities, signed owner policy and one-time approval/revocation, role-correct policy-bound mTLS hello, bounded relay circuit/queues, a Windows live publisher, and a separate Core receiver host), then provision the documented test relay for manual integration.
...
**Freshness composition decision (2026-07-18, implementation gap recorded 2026-07-19):** the receiver Core retains each last accepted Bridge capability group in a Bridge-specific read state with its provenance, receipt time, publication/session/lease/sequence identity, cadence observations, and terminal admission reason where applicable. Sender wall-clock time is diagnostic provenance only; it must not be the receiver's age clock. The current deterministic local seam accepts a receiver-observed `DateTimeOffset` and clamps a backwards value to zero; it is not yet a monotonic-clock transport implementation. The future remote host/session layer must provide a monotonic receipt clock before a live Bridge release, so host wall-clock rollback cannot extend a current window. This state remains separate from `LiveTelemetrySnapshot`, `ILiveTelemetrySink`, local history, and capture input.
