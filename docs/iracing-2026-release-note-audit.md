...
## Dynamic `CarIdx` arrays — local-context correction complete; broader evidence pending
...
- `LiveLocalStrategyContext` and `LiveLocalRadarContext` normalize `CarIdx` values as non-negative, without imposing a second fixed upper bound. Focus/player index 71 is covered directly; negative indexes are rejected. Diagnostics delegates its live-radar context checks to that same normalized path.
- This does not yet establish end-to-end dynamic-index history behavior: `HistoricalSessionAccumulator` still has legacy `< 64` guards in radar-calibration candidate handling. Do not claim history-wide or end-to-end high-index support until that follow-up is resolved and replayed.
...
## Overlay Bridge live-publisher slice — 2026-07-19 no-impact check

Reviewed the current release-note index and the latest Season 3 Patch 3 Hotfix 1 before committing the Bridge live-publisher/receiver slice. The hotfix addresses only an Acura NSX GT3 EVO 22 loading issue; it has no SDK, telemetry, session-data, or fuel-contract change. The recent Patch 3 correction to late-join team/driver lap counts remains relevant and is already handled conservatively: the Bridge publisher reads only normalized live models, gates output by an authenticated session/lease plus local in-car eligibility, and emits a hard handoff/source-unavailable boundary rather than trying to infer a driver swap from a raw field.

Patch 1's dynamic `CarIdx` array change remains the only relevant 2026 SDK-shape finding. The Bridge payload neither sends arrays nor chooses a `CarIdx` loop bound, so the 72-slot correction has no new Bridge wire/schema implication. Continue the existing capture/replay follow-up for populated high-index competitors; no raw-capture, durable-user-data, or Bridge fact-contract version change is required by this slice.
