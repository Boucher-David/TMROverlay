...
Compact edge-case telemetry artifacts, live overlay diagnostics, and Fuel V2 capture artifacts collected without raw capture are not part of the raw capture format. They are written separately under the logs root as JSON and may be included in diagnostics bundles without including `telemetry.bin`. A support bundle made during an active collection can additionally include provisional `metadata/current-*.json` observer snapshots; these have `isFinalized: false`, are never promoted or persisted, and do not alter the final raw-capture or observer artifacts.
