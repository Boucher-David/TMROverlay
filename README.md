...
- Pull-request runs upload a short-lived `tmroverlay-pr-<number>-v<version>-r<run>-a<attempt>-<sha>-installer` artifact containing the MSI created by the Velopack dry run, alongside the portable test-build artifact. It is for manual tester installation only; it is not a GitHub Release asset or an in-app updater channel.
