...
- 2026-07-13: Gate 2 now has a typed effective-capacity resolver and ordered fuel-checkpoint contract. Session info retains `DriverCarMaxFuelPct` and exact-local-driver `CarClassMaxFuelPct`; matching or single-source valid caps can resolve physical tank capacity, while missing/invalid/conflicting/same-scope-observed-above-cap evidence stays explicit and blocks advice. Observed maxima reset when car, session, or capacity-rule scope changes, so unrestricted warmup fuel cannot conflict a restricted race. Checkpoints keep first-green, current, expected-at-box, service-complete, and expected-pit-exit fuel distinct, with the pit request defined against service-complete fuel and box-to-exit burn applied once afterward. Impossible subtraction stops the dependent chain and conflicted checkpoint provenance remains typed. Existing format-version-1 sidecar/history fields carry these facts, so old null/placeholder artifacts remain compatible without a schema bump.
