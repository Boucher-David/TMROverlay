...
- When asked about overlay functionality or asked to change overlay behavior, start from the app-owned contracts before proposing or editing: inspect `tools/validation/overlay-scenario-contract.json`, `src/TmrOverlay.Core/Overlays/OverlayBehaviorDescriptorCatalog.cs`, `src/TmrOverlay.App/Overlays/BrowserSources/Assets/contracts/overlay-geometry.json`, `docs/data-contracts.md`, `docs/overlay-logic.md`, `docs/overlay-evidence-contract-testing.md`, `tools/validation/overlay-evidence-contract.mjs`, `tools/validate_overlay_screenshots.py`, `tools/browser-review/render-screenshots.mjs`, and the relevant overlay-specific logic doc. When code changes behavior, update the matching contracts, fixtures, tests, screenshot/evidence expectations, and docs in the same pass.
...
- Treat CI/build/test failures as report-first unless the user explicitly asks for fixes. Inspect available logs, annotations, artifacts, and local evidence; summarize the exact failure and classify it as test expectation, CI/tooling, or possible product behavior before changing code. Do not brute-force CI failures, do not change production code, and do not rewrite broad test expectations until the user has agreed on the intended fix.
