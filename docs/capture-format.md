...
- `fuel-v2-capture/fuel-v2-diagnostics.json` summarizes compact Fuel Calculator V2 calibration evidence when Fuel V2 capture is enabled, including app/session scope, physical tank capacity, applicable driver/class fuel-cap percentages, resolved effective event capacity with source/conflict limitation, sampled fuel/progress/lap-budget inputs, lap-burn windows, sector-burn samples, pit windows, team stint windows, and source/rejection labels. These capacity properties use fields already present in format version 1; older sidecars may retain null values or the earlier unavailable placeholder. The sidecar remains diagnostic evidence; a separate Fuel V2 importer can promote selected derived facts into `history/user/fuel-v2/` after finalization.
